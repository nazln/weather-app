...
// getCoordinates pobiera współrzędne geograficzne dla podanego miasta
func getCoordinates(city string) (*GeoResult, error) {
	// Budujemy URL zapytania do API geokodowania
	apiURL := fmt.Sprintf(
...
// getWeather pobiera aktualną pogodę dla podanych współrzędnych
...
// -------------------------------------------------------------------
// Handler HTTP - obsługa żądań
// -------------------------------------------------------------------

// handleIndex obsługuje żądania GET i POST na ścieżce "/"
func handleIndex(tmpl *template.Template) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Obsługa formularza (metoda POST - użytkownik wybrał miasto)
		if r.Method == http.MethodPost {
			city := r.FormValue("city") // Odczytujemy wybrane miasto z formularza

			// Pobieramy współrzędne dla wybranego miasta
			...
			// Przekazujemy przetworzone dane do szablonu
...
func main() {
	// Obsługa flagi -health-check (używanej przez HEALTHCHECK w Dockerfile)
	// Wysyła żądanie GET do endpointu /health i kończy proces z kodem 0 (OK) lub 1 (błąd)
	...
	// Logowanie informacji startowych kontenera (wymagane przez zadanie)
	...
	// Wczytujemy szablon HTML z pliku
	tmpl, err := template.ParseFiles("templates/index.html")
	if err != nil {
		log.Fatalf("Błąd wczytywania szablonu: %v", err)
	}

	// Rejestrujemy handler dla ścieżki głównej
	http.HandleFunc("/", handleIndex(tmpl))

	// Endpoint zdrowia kontenera (używany przez HEALTHCHECK w Dockerfile)
